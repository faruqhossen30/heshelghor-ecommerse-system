    ...
    <div class="col-12">
        <div class="card mt-3">
            <div class="card-header border-bottom bg-transparent">
                <h5 class="header-title mb-0">Product promotion</h5>
            </div>
            <div class="card-body">
                <form action="{{route('productpromotion')}}" method="POST">
                    @csrf
                    <h5 class="mb-1 mt-3 mt-md-0 font-14">Search for featured product.</h5>
                    <select name="product_id[]" class="multi-select" multiple="" id="my_multi_select3">
                        {{-- <option value="AF">Afghanistan just for test</option> --}}
                        @foreach ($products as $product)
                        <option value="{{$product->id}}"
                            @if ($promotion_products && in_array($product->id, $promotion_products)) selected @endif
