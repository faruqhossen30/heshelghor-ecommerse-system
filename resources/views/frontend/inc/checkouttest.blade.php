...
@if (count($couriers) > 0)
    ...
    @foreach ($couriers as $courier)
        ...
        @if ($courier->code == 'HESHEL')
            ...
            <option value="35">{{ $courier->name }} - 35 TK </option>
        ...
        @if ($courier->code == 'PAPERFLY')
            ...
            <option value="50">{{ $courier->name }} - 50 TK </option>
            @else
            <option value="110">{{ $courier->name }} - 110 TK </option>
            @endif
        @endif
        ...
        @if ($courier->code == 'redx')
            @if (in_array($prdoduct_upazila_id, $dhaka))
                <li>
                    <div class="custom-radio">
                        <input type="radio" id="deliverrat{{ $courier->id }}" name="delivery_system" value="50">
                        <label class="custom-control-label"
                            for="deliverrat{{ $courier->id }}"><strong>{{ $courier->name }}</strong>
                            <small>
                                - Charge: ৳50</small>
