...
@if(count($couriers) > 0)
            ...
            @foreach ($couriers as $courier)
                ...
                @if ($courier->code == 'HESHEL')
                    <option value="35" data-curierid="{{$courier->id}}" data-courierpackegedesc="Heshel Express">{{$courier->name}}  - ৳35</option>
                @endif
                {{-- Heshel Express End --}}
                ...
                @if ($courier->code == 'PAPERFLY')
                    ...
                    <option value="50" data-curierid="{{$courier->id}}" data-courierpackegedesc="Paper Fly">{{$courier->name}}  - ৳50</option>
                    @else
                    <option value="110" data-curierid="{{$courier->id}}" data-courierpackegedesc="Paper Fly">{{$courier->name}}  - ৳110</option>
                    @endif
                @endif
